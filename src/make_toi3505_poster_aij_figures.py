...
def figure_dmag_rms(path: Path) -> dict:
    ...
    rms = np.array([float(r["night_robust_scatter_ppt"]) for r in rows])
    ...
    limit = np.array(
        [
            3.0 * float(r["historical_window_depth_error_ppt"])
    ...
    axis.tick_params(labelsize=13)
    axis.set_title(
        f"Nearby stars within 60 arcsec: {int(cleared.sum())} of {len(rows)} cleared",
        fontsize=16,
        color="black",
    )
...
def main() -> None:
    ...
    summary = {"seeing": seeing, "comparison_field": field, "dmag_rms": dmag}
    (OUT / "v2_figure_summary.json").write_text(json.dumps(summary, indent=2))

    print(json.dumps(summary, indent=2))
    print(f"\nWrote three figures to {OUT}")
